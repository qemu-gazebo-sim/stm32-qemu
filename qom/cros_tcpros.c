...
#include "qom/cros_api.h"
#include "qom/cros_tcpros.h"
#include "qom/cros_defs.h"
#include "qom/tcpros_tags.h"
#include "qom/tcpros_process.h"
#include "qom/dyn_buffer.h"
#include "qom/cros_log.h"
...
static TcprosParserState readPublicationHeader( TcprosProcess *p, uint32_t *flags )
{
  ...
  *flags = 0x0;
  ...
  while ( bytes_to_read > 0)
  {
    ...
    if( field_len )
    {
      // http://wiki.ros.org/qom/TCPROS doesn't mention it but it's sent anyway in ros groovy
...
void cRosMessagePreparePublicationHeader( CrosNode *n, int server_idx )
{
  ...
  // http://wiki.ros.org/qom/TCPROS doesn't mention to send message_definition and topic_name
